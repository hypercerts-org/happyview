...
const [forest, river, blobLocation, literalLocation, noRelations, profileOnly, organizationOnly] = locationRecords;
...
test('listLocations search trims input and matches literal case-insensitive text within name or description', async () => {
  const result = await get('app.certified.location.listLocations', { search: '  COMMUNITY forest  ', limit: 100 });
  assert.deepEqual(result.locations.map(({ uri }) => uri), [forest.uri]);
});
...
test('empty locationType matches the fixture value', async () => {
  ...
  assert.deepEqual(result.locations.map(({ uri }) => uri), [literalLocation.uri]);
});

test('listLocations search treats SQL wildcards and one backslash literally', async () => {
  const result = await get('app.certified.location.listLocations', { search: String.raw`  100%_\path and + spaces  ` });
  assert.deepEqual(result.locations.map(({ uri }) => uri), [literalLocation.uri]);
});

test('listLocations search does not match when the query has an extra backslash', async () => {
  const result = await get('app.certified.location.listLocations', { search: String.raw`100%_\\path` });
  assert.deepEqual(result.locations, []);
});
...
test('listLocations paginates deterministically in both directions without changing filters', async () => {
  for (const direction of ['asc', 'desc']) {
    ...
    const expected = [forest, river, blobLocation, literalLocation, noRelations].sort((a, b) => {
...
test('listLocations cursor stores the UTC database instant, not the record timezone', async () => {
  const page = await get('app.certified.location.listLocations', {
    uris: [forest.uri, river.uri, blobLocation.uri, literalLocation.uri, noRelations.uri],
    sortDirection: 'asc', limit: 4,
  });
  assert.equal(page.locations.at(-1).uri, literalLocation.uri);
