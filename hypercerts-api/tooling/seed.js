...
import { seedSql } from '../tests/fixtures/records.js';
import { badDateSeedSql } from '../tests/fixtures/bad-location-dates.js';
...
function sqlInput(statements, types = 'text, text, text, text, jsonb, text, text') {
  return [
    ...
    `PREPARE happyview_location_fixture (${types}) AS ${statements[0].sql};`,
    ...statements.map(({ params }) => `EXECUTE happyview_location_fixture(${params.map(quote).join(', ')});`),
    'DEALLOCATE happyview_location_fixture;',
  ].join('\n');
}

export function buildSeedInput(env = process.env) {
  disposableTarget(env);
  return sqlInput(seedSql({ disposableTestTarget: true }));
}

export function buildBadDateSeedInput(env = process.env) {
  disposableTarget(env);
  return sqlInput(badDateSeedSql({ disposableTestTarget: true }), 'text, text, text, text, jsonb, text, text, text');
}
