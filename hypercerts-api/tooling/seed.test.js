...
test('seed input selects only explicitly supplied non-location rows', () => {
  const custom = {
    ...
    record: { $type: 'org.hypercerts.custom', name: "O'Brien \\woods" },
    cid: 'bafycustom', indexedAt: '2025-01-02T03:04:05.000Z',
  };
  const sql = buildSeedInput(safeSeedEnv, [custom]);
  assert.match(sql, /PREPARE happyview_fixture \(text, text, text, text, jsonb, text, text\)/);
  ...
  assert.match(sql, /\\\\woods/);
  assert.doesNotMatch(sql, /app\.certified\.location|app\.certified\.actor/);
  assert.throws(() => buildSeedInput({ ...safeSeedEnv, PGHOST: 'db.example.com' }, [custom]), /PGHOST explicitly/);
});

test('seed input refuses empty row sets instead of generating invalid PREPARE SQL', () => {
  assert.throws(() => buildSeedInput(safeSeedEnv, []), /at least one.*row/i);
...
test('bad-date seed input uses eight parameters only for the fourteen isolated fixtures', async () => {
  ...
  assert.equal((sql.match(/^EXECUTE happyview_fixture\(/gm) ?? []).length, 14);
  ...
  assert.match(normal, /PREPARE happyview_fixture \(text, text, text, text, jsonb, text, text\)/);
...
test('bad-date CLI is opt-in, passes eight-parameter SQL to selected executable, and rejects unknown flags', async () => {
  await withCopiedSeedModule((script) => withFakeExecutables(async ({ selected, root }) => {
    ...
    for (const row of expectedRows) {
      ...
      assert.equal(matching.length, 1, `expected exactly one default CLI row for ${row.uri}`);
      const recordJson = JSON.stringify(row.record).replaceAll('\\', '\\\\').replaceAll("'", "''");
      assert.equal(matching[0], `EXECUTE happyview_fixture(E'${row.uri}', E'${row.did}', E'${row.collection}', E'${row.rkey}', E'${recordJson}', E'${row.cid}', E'${row.indexedAt}');`);
    }
    assert.doesNotMatch(normalSql, /did:plc:baddatefixturesexamplexx/);
